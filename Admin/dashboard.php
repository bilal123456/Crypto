...
<nav class="navbar navbar-expand-md   fixed-top" style="background-color: #225269;color: white;">
     <button type="button" id="sidebarCollapse" class="btn btn-info">
      ...
      <tr>
        ...
        <?php echo $sql = "select * from customer";
        ...
        if(mysqli_num_rows($run)> 0)
        {
            ...
            <td><a href="editcustomer.php?id=<?php echo $row['id']?>" class="btn btn-info btn-xs" style="margin:10px;">Edit</a> <a class="btn btn-primary btn-xs" style="margin: 10px;" href="deletecustomer.php?id=<?php echo $row['id']?>">Delete</a><button id="showinvoice" data-id="<?php echo $row['username'] ?>" class="btn btn-primary">See Rafferal</button><button id="showdeposit" data-id="<?php echo $row['id'] ?>" class="btn btn-success">Deposit</button><button id="showwithdraw" data-id="<?php echo $row['id'] ?>" class="btn btn-danger">Withdraw</button></td>
            </tr>
...
@media (max-width: 768px) {
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on('click','#showdeposit',function(){
               
                $('#myModaldeposit').modal('show');
                var id =$(this).attr("data-id");
                $.ajax({
                    url:"fetchdeposit.php",
                    method:"post",
                    data:{id:id},
                    success:function(data)
                    {
                        $('#resultdeposit').html(data);
                       // alert(data);
                    }
                })
            });
            ...
            $(document).on('click','#showwithdraw',function(){
               
                $('#myModalwithdraw').modal('show');
                $('#valuechanges').hide();
                var id =$(this).attr("data-id");
                $.ajax({
                    url:"fetchwithdraw.php",
                    method:"post",
                    data:{id:id},
                    success:function(data)
                    {
                        $('#resultwithdraw').html(data);
                       //test = $("td:contains($(this).val())").text();
                     //  alert(test);
                    }
                })
            });
