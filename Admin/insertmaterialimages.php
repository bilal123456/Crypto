 ...
 <div class="container" class="form-horizontal ">
    <div class="row"  style="position: relative;top: 50px;">
      <div class="col-sm-4 offset-md-3">
        <div class="card card-signin my-2  mr-8 p-0 r-3">
          <div class="card-body">
            <h5 class="card-title text-center">Insert Image Or Vedios</h5>
            <form method="post" class="form-signin" enctype="multipart/form-data">
              <div class="form-label-group">
                <label>Image</label>
                 
                <input name="image" type="file" id="inputEmail" class="form-control form-input">
               
              </div>
<br>
<label>Vedio</label>
              <div class="form-label-group">
                <input placeholder="password" type="file" name="vedio" id="inputPassword" class="form-control form-input">
...
<br>
    ...
    transition: all .4s;
    width: 90%;
    z-index: 9999!important;">Upload</button>
...
if(isset($_POST['login']))
{
    ...
    $imagename = $_FILES['vedio']['name'];
    $tmpimage = $_FILES['vedio']['tmp_name'];

    //insert image 
    $insertimage = "insert into materialimage(name) values('".$name."')";
    $runimage = mysqli_query($link,$insertimage);
    if($runimage)

    {
        $uploadimage = "Vedio/images/";
        move_uploaded_file($tmp, $uploadimage . $name);
    }
    echo $insertvedio = "insert into vedio(name) values('".$imagename."')";
    $vedio = mysqli_query($link,$insertvedio);

    if($vedio)
    {
        $uploadimage = "Vedio/";
        move_uploaded_file($tmp, $uploadimage . $imagename);
    }
